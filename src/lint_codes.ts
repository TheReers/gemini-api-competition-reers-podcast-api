...
(async function lintFiles() {
    ...
    if (warnings.length) {
        const error = 'The are some warnings in the code:'
        console.log('\x1b[33m', '\x1b[4m', error)
    ...
    if (errors.length) {
        ...
        throw { message: 'Fix the linting issues above' }
    } else {
        successLog('No linting issues found ✅.')
    }
}()).catch((e) => {
    errorLog(e.message)
    process.exit(1)
